...
3. Блок контекста из найденных чанков (`rag/generation.py`).
4. System prompt = RAG-шаблон + статические главы 00/13 + контекст.
5. `ChatCompletionClient` генерирует ответ.
6. Trace уходит по SSE во время запроса; сохраняется в metadata сообщения.
