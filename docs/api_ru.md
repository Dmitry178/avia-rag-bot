...
Структура тестов: [backend/tests/README_RU.md](../backend/tests/README_RU.md). Правило Cursor для новых эндпоинтов: `.cursor/rules/backend-api-tests.mdc`.

---

## Покрытие API-тестами

На каждый эндпоинт — **2–3 HTTP-теста** в `backend/tests/api/` (внешние зависимости мокаются при необходимости).

| Метод | Путь | Тесты | Статус |
|-------|------|-------|--------|
...
| `GET` | `/api/chats/{chat_id}` | `test_chat.py` | покрыт |
| `PATCH` | `/api/chats/{chat_id}` | `test_chat.py` | покрыт |
| `DELETE` | `/api/chats/{chat_id}` | `test_chat.py` | покрыт |
| `POST` | `/api/chats/{chat_id}/close` | `test_chat.py` | покрыт |
| `POST` | `/api/chats/{chat_id}/messages` | `test_chat.py` | покрыт |
| `PATCH` | `/api/chats/{chat_id}/messages/{message_id}` | `test_chat.py` | покрыт |
