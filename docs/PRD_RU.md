...
3. RAG подтягивает decision tree 16.2.
4. Ответ — пошаговый алгоритм.
5. В критической фазе бот напоминает вызвать службу безопасности.
...
- Backend: ETL, FAISS, multi-lane RAG, чаты, LLM/RAG ответы, SSE trace, prompt guards
- Frontend: трёхколоночный UI, настройки RAG/LLM, trace viewer, i18n, темы
