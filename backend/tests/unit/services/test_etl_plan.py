...
def _draft(node_id: str, content: str, content_hash: str) -> ChunkDraft:
    """
    Build a minimal chunk draft for ingest planning tests.
    """
...
def _chunk(node_id: str, content_hash: str, chunk_id: int = 0) -> ChunkMeta:
    """
    Build a minimal stored chunk row with a stable FAISS row index.
    """
...
def test_plan_reuses_unchanged_chunks_from_faiss() -> None:
    """
    Unchanged drafts should reuse existing FAISS vectors and skip embedding.
    """
...
def test_plan_embeds_changed_and_new_chunks() -> None:
    """
    Changed and newly added drafts should both be scheduled for embedding.
    """
...
def test_plan_marks_removed_chunks() -> None:
    """
    Drafts missing a previously stored node_id should count as removed chunks.
    """
...
def test_plan_uses_checkpoint_vectors_before_existing() -> None:
    """
    Checkpoint vectors keyed by content_hash should win over stored FAISS rows.
    """
...
def test_plan_rebuild_embeds_everything() -> None:
    """
    Rebuild mode should still reuse checkpoint vectors but skip FAISS reuse.
    """
